...
def get_user_meal_history(user_id: str) -> str:
    ...
    try:
        ...
        meals = db.query(MealLog).filter(MealLog.user_id == user.id).order_by(MealLog.meal_time.desc()).limit(7).all()
        if not meals:
            return "No meal history found."
        return "\n".join(f"{meal.meal_time.date()}: {meal.summary}" for meal in meals)
    finally:
        db.close()
